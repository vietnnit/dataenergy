...
<script runat="server">
	...
	public override void SetConfig()
	{
        ...
        if ((Session["Admin_Username"] != null || Session["Admin_Username"].ToString() != "") && !Session["Admin_Username"].ToString().Equals("administrator"))
        {
            string rootURL = ResolveUrl("~") + "userfile/User/";
            string pathToCreate = rootURL + Session["Admin_Username"].ToString();
            ...
            BaseUrl = ResolveUrl("~") + "userfile/User/" + Session["Admin_Username"].ToString() + "/";
        }
        else
            BaseUrl = ResolveUrl("~") + "userfile/";
		...
		PluginSettings.Add("ImageResize_smallThumb", "200x200" );
		PluginSettings.Add("ImageResize_mediumThumb", "400x320" );
		PluginSettings.Add("ImageResize_largeThumb", "800x450" );
		...
		// feature.
		Images.MaxWidth = 4096;
		Images.MaxHeight = 2160;
		Images.Quality = 80;
        ...
        //Create Folder DateTime
        DateTime now = DateTime.Now;
        if (!System.IO.Directory.Exists(Server.MapPath(type.Url + now.Year)))
        {
            System.IO.Directory.CreateDirectory(Server.MapPath(type.Url + now.Year));
        }
        if (!System.IO.Directory.Exists(Server.MapPath(type.Url + now.Year + "/" + now.Month)))
        {
            System.IO.Directory.CreateDirectory(Server.MapPath(type.Url + now.Year + "/" + now.Month));
        }
       ...
       // DateTime now = DateTime.Now;
        ...
        type.MaxSize = 0;
        type.AllowedExtensions = new string[] { "mp4", "flv", "wma", "mp3", "wmv" };
        type.DeniedExtensions = new string[] { };
