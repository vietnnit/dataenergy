...
{
    public class EnterpriseHistoryDao : EntityDao<EnterpriseHistoryBO>
    {
        ...
        public IList<EnterpriseHistory> FindList(int Year, int AreaId, int SubAreaId, int OrgId, int ProvinceId, int DistrictId, bool? blActive, string keyword, PagingInfo paging)
        {
            ...
            try
            {
                list = new Db().GetList<EnterpriseHistory>("Get_Enterprise_Find", parameter, System.Data.CommandType.StoredProcedure);
                return list;
            }
            catch (Exception ex)
            {
        ...
        public IList<EnterpriseHistory> GetEnterpriseByOrg(int OrgId)
        {
            ...
            try
            {
                list = new Db().GetList<EnterpriseHistory>("SELECT * FROM " + new EnterpriseHistoryBO().TableName + " WHERE OrganizationId=@OrgId ORDER BY Title ASC", parameter, System.Data.CommandType.Text);
                return list;
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetDataEnterpriseByOrg(int OrgId)
        {
            ...
            try
            {
                return new Db().GetDataTable("SELECT * FROM " + new EnterpriseHistoryBO().TableName + " WHERE OrganizationId=@OrgId ORDER BY Id DESC", parameter, System.Data.CommandType.Text);
                //return list;
            }
            catch (Exception ex)
            {
        ...
        public int GetNoAccount(int OrgId)
        {
            ...
            DbParameter[] parameter = new DbParameter[1];
            parameter[0] = new DbParameter("@OrgId", OrgId);
            DataTable dt = new Db().GetDataTable("SELECT COUNT(*) as NoAccount FROM " + new EnterpriseHistoryBO().TableName + " WHERE OrganizationId=@OrgId", parameter, System.Data.CommandType.Text);
        ...
        public DataTable GetDNByName(string strName, int OrgId)
        {
            ...
            DataTable dt = new Db().GetDataTable("SELECT Id, Title, OrganizationId FROM " + new EnterpriseHistoryBO().TableName + " WHERE OrganizationId=@OrgId AND Title=@Title", parameter, System.Data.CommandType.Text);
            return dt;
